...
SCRAPERAPI_KEY = f70288fff3436aca9b43f553e6676805  # 예: abc123456789
...
def get_transcript(videoId: str = Query(...)):
    try:
        ...
        if not match:
            return {"error": "Failed to find captionTracks."}

        tracks = json.loads(match.group(1))
        transcript_url = tracks[0].get("baseUrl")
        if not transcript_url:
            return {"error": "Transcript URL missing"}

        resp = requests.get(transcript_url)
        if resp.status_code != 200:
            return {"error": f"Transcript request failed: {resp.status_code}"}
